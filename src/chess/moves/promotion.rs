...
use crate::{CursorWorldCoordinates, chess::{BOARD_LENGTH, ChessState, PIECE_SIZE, board::Board, direction::Direction, piece::{Piece, PieceColor}, position::{Position, SyncTransformWithPosition}, preset_pieces::{bishop, knight, queen, rook}, setup::spawn_piece}, layers};
...
#[derive(Event)]
pub struct AttemptPromotionEvent(pub Entity, pub Position, pub Position);
...
pub fn attempt_promotion(event: On<AttemptPromotionEvent>, mut commands: Commands, mut next_state: ResMut<NextState<ChessState>>, asset_server: Res<AssetServer>,
    ...
    let translation = position.to_translation() + vec2(dir.dfile as f32, dir.drank as f32) * PIECE_SIZE * 1.5;
...
pub fn check_promotion_option_clicked(mut commands: Commands, mut next_state: ResMut<NextState<ChessState>>, cursor: Res<CursorWorldCoordinates>, input: Res<ButtonInput<MouseButton>>,
    mut board: Single<(Entity, &mut Board)>, pieces: Query<(Entity, &Position, &Transform), (With<Piece>, With<PromotingPiece>)>,
    promotion_options: Query<(Entity, &Position, &PromotionOption)>) {
    ...
    for (promotion_option_entity, &promotion_option_position, promotion_option) in promotion_options {
        if cursor == promotion_option_position {
            let (board_entity, ref mut board) = *board;
            ...
            println!("{piece}");
            commands.entity(piece).log_components();
